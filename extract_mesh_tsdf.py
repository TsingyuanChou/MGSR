...
import random
import torch
from geo_scene import Scene
import os
from tqdm import tqdm
from os import makedirs
from geo_gaussian_renderer import render
import torchvision
from geo_utils.general_utils import safe_state
from argparse import ArgumentParser
from geo_arguments import ModelParams, PipelineParams, get_combined_args
from geo_gaussian_renderer import GaussianModel
import numpy as np
...
def tsdf_fusion(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    with torch.no_grad():
        for _, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            mask = view.mask.cuda()
            #depth = rendering[6:7, :, :]
            depth = rendering['surf_depth'] 
            alpha = rendering['rend_alpha']
            rgb = rendering['render']
            #alpha = rendering[7:8, :, :]
            #rgb = rendering[:3, :, :]
            
            depth = depth * mask[0].unsqueeze(0)
